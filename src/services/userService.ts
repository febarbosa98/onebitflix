...
updatePassword: async (id: number, password: string) =>{
    ...
    getKeepWatchingList: async(id: number) => {
        const userWitchingEpisodes = await User.findByPk(id, {
            include: {
        ...
        if(!userWitchingEpisodes) throw new Error('Usuario não encontrado')

        const keepWatchingList = filterLastEpisodesByCourse(userWitchingEpisodes.Episodes!)
        // @ts-ignore
